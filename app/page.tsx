...
const Home = () => {
  ...
  const contractsData = contracts?.slice(0, 5)?.map(contract => ({
    id: contract?.id,
  ...
  const paymentsData = payments?.slice(0, 5)?.map(payment => ({
    id: payment?.id,
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {upcomingEventsSummary.length} {upcomingEventsSummary.length === 1 ? 'evento' : 'eventos'} nos próximos 15 dias
              </p>
            </div>
        ...
        <div className="bg-card border border-border rounded-lg p-6">
          ...
          <div className="mt-4 h-2 w-full bg-muted rounded-full overflow-hidden">
            {(() => {
              ...
              const paid = (payments || []).filter(p => String(p?.status).toLowerCase() === 'paid').reduce((s, p) => s + (parseFloat(p?.amount) || 0), 0);
