...
export const paymentService: PaymentServiceInterface = {
  ...
  async getAll() {
    // Always use safe manual assembly to avoid PostgREST 400s due to missing FK metadata
    try {
      ...
      const { data: events } = eventIds.length
        ? await supabase.from('events').select('*').in('id', eventIds.map(String) as string[])
        : ({ data: [] } as any);
      ...
      const { data: profiles } = producerProfileIds.length
        ? await supabase.from('profiles').select('*').in('id', producerProfileIds.map(String) as string[])
        : ({ data: [] } as any);
      ...
      const eventsMap = (events || []).reduce((acc: Record<string, any>, ev: any) => {
        ...
        const producer = producerRaw
          ? { ...producerRaw, profile: (profiles || []).find((pr: any) => pr.id === producerRaw.profile_id) || null }
          : null;
        const eds = (eventDjs || [])
      ...
      const enriched = (payments || []).map((p: any) => ({
        ...p,
        event: p.event_id ? eventsMap[p.event_id] || null : null,
      }));
  ...
  async getByDJ(djId: string): Promise<EnrichedPayment[]> {
    try {
      ...
      const { data: payments } = await supabase
        .from('payments')
        .select('*')
        .in('event_id', eventIds.map(String) as string[])
        .order('created_at', { ascending: false });

      const eventIdsAll = Array.from(new Set((payments as any[]).map((p) => p.event_id).filter(Boolean)));
      const { data: events } = eventIdsAll.length
        ? await supabase.from('events').select('*').in('id', eventIdsAll.map(String) as string[])
        : ({ data: [] } as any);
      const djIds = Array.from(new Set((events || []).map((ev: any) => ev.dj_id).filter(Boolean)));
      const { data: djs } = djIds.length
        ? await supabase.from('djs').select('*').in('id', djIds.map(String) as string[])
      ...
      const { data: profiles } = producerProfileIds.length
        ? await supabase.from('profiles').select('*').in('id', producerProfileIds.map(String) as string[])
        : ({ data: [] } as any);
      const { data: eventDjs } = eventIdsAll.length
        ? await supabase.from('event_djs').select('*').in('event_id', eventIdsAll.map(String) as string[])
        : ({ data: [] } as any);
      const extraDjIds = Array.from(new Set((eventDjs || []).map((ed: any) => ed.dj_id).filter(Boolean)));
      const { data: extraDjs } = extraDjIds.length
        ? await supabase.from('djs').select('*').in('id', extraDjIds.map(String) as string[])
      ...
      if (lower.includes('failed to fetch') || lower.includes('networkerror') || lower.includes('network error') || lower.includes('typeerror')) {
        ...
        console.error('Error fetching DJ payments:', msg);
      }
      return [];
    }
  },
  ...
  async getByProducer(producerId: string): Promise<EnrichedPayment[]> {
    try {
      ...
      const eventIdsAll = Array.from(new Set((payments as any[]).map((p) => p.event_id).filter(Boolean)));
      const { data: eventsFull } = eventIdsAll.length
        ? await supabase.from('events').select('*').in('id', eventIdsAll.map(String) as string[])
        : ({ data: [] } as any);
      ...
      if (lower.includes('failed to fetch') || lower.includes('networkerror') || lower.includes('network error') || lower.includes('typeerror')) {
        ...
        console.error('Error fetching producer payments:', msg);
      }
      return [];
    }
  },
